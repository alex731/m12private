...
<div id="YMapsID-3050" style="width:450px;height:350px"></div>
	...
	public static function getPhotosGallery($photos,$photo_path) {
		
		$html = '<ul class="gallery clearfix">';
		$i = 2;
		foreach ($photos as $photo) {
			$html.='<li><a href="'.$photo_path.$photo['name'].'" rel="prettyPhotoPhoto[gallery2]" title="'.$photo['description'].'"><img src="'.$photo_path.$photo['name'].'_prev" alt="'.addslashes($photo['title']).'" class="border"/></a></li>';
			if ($i==0) {
				//$html.='<br>';
				$i = 2;
			}
