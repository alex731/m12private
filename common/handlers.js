...
function uploadSuccess(file, serverData) {
	try {
		var progress = new FileProgress(file,  this.customSettings.upload_target);
		var indTenement = serverData.indexOf('tenement');		
		var indFlat = serverData.indexOf('flat');
		if (indTenement>=0) {			
			addImage(serverData.substring(indTenement+9),'Tenement');
			progress.setStatus("Превью созданы.");
			progress.toggleCancel(false);
		}
		else if (indFlat>=0) {
			addImage(serverData.substring(indFlat+5),'Flat');
			progress.setStatus("Превью созданы.");
			progress.toggleCancel(false);
		}
		else {
...
function addImage(file_name,kind) {
