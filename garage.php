...
function add($s) {
	if (empty($_POST)) {		
		...
		if (!isset($errors['is_error'])&&!isset($errors['captcha']['is_error'])) {			
			$garage_id = Land::addStatic($_POST);			
			setcookie('contacts',stripslashes($_POST['contacts']));
			$_SESSION['last_garage_id'] = $garage_id;			
			if ($garage_id > 0) header("Location: /garage.html?action=view&id=".$garage_id);			
			exit();
		}
		else {
...
function edit($s) {	
	...
	$garage->getFull($id);
	//загрузка дома	
	if (empty($_POST) && $garage->id>0) {		
		if (isset($_SESSION["admin"])) $_SESSION['last_garage_id'] = $id; 	
		if ($id!=$_SESSION['last_garage_id'] && !$_SESSION['user_id'] && !$_SESSION["admin"]) {
			header("Location: /index.html");
			exit();
		}		
		if (isset($_SESSION['user_id']) && $garage->user_id != $_SESSION['user_id'] && !$_SESSION["admin"] 
			&& $id!=$_SESSION['last_garage_id']) {
			header("Location: /index.html");
			exit();
		}		
		$block_html = Html::pageLandEdit($garage);
		$s->assign("YANDEX_KEY",YANDEX_KEY);
		
		$s->assign("city_id",$garage->city_id);
		$s->assign("city",$garage->city);				
		if (isset($_SESSION['admin'])) $s->assign("is_admin",$_SESSION['admin']);		
		$s->assign("block_html",$block_html);
		$s->display("garage_edit.tpl");
	}
	//обновление
	elseif (isset($_POST) && $_REQUEST['id']>0) {
		$garage = new Land();		
		$errors = $garage->checkForm($_REQUEST);
		$id = intval($_REQUEST['id']);
		if ($id!=$_SESSION['last_garage_id'] && !$_SESSION['user_id'] && !$_SESSION["admin"]) {
			header("Location: /index.html");
			exit();
		...
		if (!isset($_SESSION['user_id'])) {
			...
			$garage->getFull($id,"f.user_id='{$_SESSION['user_id']}'");
			if ($garage->user_id != $_SESSION['user_id'] && !$_SESSION["admin"] && $id!=$_SESSION['last_garage_id']) {
				header("Location: /index.html");
				exit();
			}
		}				
		if (!isset($errors['is_error']) && $garage->id > 0) {						
			$garage->update($_POST);
			$photo_garage_path = $garage->getPhotoPath();
			if (!is_dir($photo_garage_path)) {			
				mkdir($photo_garage_path,0777);
				chmod($photo_garage_path,0777);						
			}
			//добавляем новые фото
			if (isset($_POST['photo_garage']) && is_array($_POST['photo_garage'])) {
				foreach ($_POST['photo_garage'] as $fname) {
					$fname = clearTextData($fname);
					if ($fname!='') $garage->addPhoto($fname,$_POST);
				}
			}
			//редактируем существующие фото
			if (isset($_POST['photo_garage_exist']) && is_array($_POST['photo_garage_exist'])) {
				foreach ($_POST['photo_garage_exist'] as $fname) {
					$fname = clearTextData($fname);
					if ($fname!='') $garage->editPhoto($fname,$_POST);
				}
			}			
			header("Location: /garage.html?action=view&id=".$garage->id);
			exit();
		}
		else {
			echo "error";
			$id = intval($_REQUEST['id']);			
			$garage->getFull($id);				
			$block_html = Html::pageLandEdit($garage,$errors);
			$s->assign("YANDEX_KEY",YANDEX_KEY);
...
function apply($s) {
	$id = intval($_REQUEST['id']);
	if ($id!=$_SESSION['last_garage_id']) {		
		//header("Location: /index.html");
		echo "$id!={$_SESSION['last_garage_id']}";
		exit();
	}
	$garage = new Land();
	$status = REALTY_STATUS_APPLY;
	$user_cond = (isset($_SESSION['user_id'])) ? ' AND user_id='.$_SESSION['user_id'] : '';
	$garage->updateBy("id=$id",array('status'=>$status));	
...
function _filter($status) {
	...
	*/
	...
	if (isset($_REQUEST['f_garage'])) {
		$f_garage = intval($_REQUEST['f_garage']);
		if ($f_garage>5) $f_garage=0;
		if ($f_garage>0) $add_sql .= " AND f.type_id =".($f_garage-1);		
		$filter['garage'] = $f_garage; 
	}
...
function map($s) {
	...
	$block_html = Html::getBlock('Квартиры на продажу на карте Йошкар-Олы',Html::pagegarageMap());
	$s->assign("block_html",$block_html);
	$s->display("garage_map.tpl");
}
